...
def main():
    ...
    # num_of_batches = NUMBER_OF_BITS_PRODUCED / (batch_size * )
...
def run_experiment(train_and_eval):
    ...
    train_and_eval(INPUT_SIZE['default'], GEN_WIDTH['default'], OUTPUT_SIZE['default'], MAX_VAL['default'],
                   ...
                   OPP_OPT(LEARNING_RATE['default'], BETA1, BETA2), steps, eval_data)
    # vary input size
    ...
    for learning_rate in LEARNING_RATE['options']:
        train_and_eval(INPUT_SIZE['default'], GEN_WIDTH['default'], OUTPUT_SIZE['default'], MAX_VAL['default'],
                       ...
                       OPP_OPT(learning_rate, BETA1, BETA2), steps, eval_data)


def run_discgan(input_size, gen_width, output_size, max_val, batch_size, gen_opt, opp_opt, steps, eval_data):
    ...
    for i in range(EXPERIMENT_REPETITIONS):
        ...
        files.write_numbers_to_ascii_file(eval_out, SEQN_DIR + str(i) + '_' + 'gan.txt')
...
def run_predgan(input_size, gen_width, output_size, max_val, batch_size, gen_opt, opp_opt, steps, eval_data):
    ...
    for i in range(EXPERIMENT_REPETITIONS):
        print('GAN - experiment ' + str(i) + ' -----')
        predgan.train(batch_size, steps)
        eval_out = predgan.predict(eval_data, batch_size)
        files.write_numbers_to_ascii_file(eval_out, SEQN_DIR + str(i) + '_' + 'predgan.txt')

...
from utils.operation_utils import detach_last, set_trainable, flatten_irregular_nested_iterable
...
PRE_EPOCHS = 0 if not PRETRAIN else 500 if HPC_TRAIN else 5  # number of epochs for pre-training
...
def run_discgan():
    ...
    # pre-train Diego
    diego_x, diego_y = get_sequences(jerry, get_inputs(BATCH_SIZE * BATCHES, OUTPUT_RANGE)[:-1], OUTPUT_SIZE, OUTPUT_RANGE)
    if not HPC_TRAIN:
        print_pretrain(diego_x, diego_y)
    plot_pretrain_loss(diego.fit(diego_x, diego_y, BATCH_SIZE, PRE_EPOCHS, verbose=1), PLOT_DIR + 'diego_pretrain_loss.pdf')
    ...
    plot_network_weights(flatten_irregular_nested_iterable(jerry.get_weights()), PLOT_DIR + 'jerry_weights.pdf')
...
def run_predgan():
    ...
    # pretrain priya
    print('PRETRAINING ...')
    priya_x, priya_y = detach_last(janice.predict(get_inputs(BATCH_SIZE * BATCHES, OUTPUT_RANGE)))
    plot_pretrain_loss(
    ...
    print('TRAINING ...')
    janice_x = get_inputs(BATCH_SIZE * BATCHES, OUTPUT_RANGE)[0]
